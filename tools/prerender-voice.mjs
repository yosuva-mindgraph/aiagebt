...
/* ============================================================================
   ...
   Nothing is typed twice. The corpus is READ from the sources at run time:
     ...
     · --scope all adds spokenForm(a) of every entry in src/knowledge.js, plus
       DONT_KNOW — the spoken form, not the answer HTML, because src/app.js
       speaks spokenForm(html) and a clip rendered from the markup would be
       audio of a string nobody ever says. It is 40 answers and about 15 MB of
       base64, and it is what the CONTAINER ships: an image built with a
       complete all-scope payload needs no ElevenLabs key at runtime, because
       there is no speakable text left for it to synthesise. build.js
       --require-voice is the assertion that this actually held.
...
function corpus() {
  ...
  const add = (text, tier, from) => {
    ...
    if (seen) {
      if (seen.text !== t) {
        // A 64-bit space and 112 strings: this should never fire. If it ever
  ...
  /* The one answer that is NOT an entry in KB, and the one most likely to be
     heard. src/ask.js returns DONT_KNOW verbatim whenever retrieval lands under
     CONFIDENCE_FLOOR — i.e. every off-script question a viewer asks — and
     src/app.js speaks it through the identical path as any other answer. It is
     declared outside the KB array, so KB.forEach above has never reached it.

     Without this line the deck answers everything it KNOWS in the real voice
     and everything it does NOT know in the robotic browser one, which is both
     the most audible failure available and precisely backwards. One clip. */
  add(spokenForm(DONT_KNOW), 'answer', 'kb (dont-know)');
...
function pack(items, params) {
  ...
  const note = `${n}/${items.length} clips · ${minutes} min · ${params.format} · ${params.model} · scope ${SCOPE}`;
  /* `corpus` travels WITH the payload, and it is the field build.js --require-voice
     reads. `n` alone cannot answer the question that matters — a file holding 88
     clips is indistinguishable from a complete one unless you also know the corpus
     was 112. That gap is not hypothetical: editing a narration line changes its
     text, which changes its voiceClipKey, which silently drops it out of the cache
     and out of this payload. The build stays green and exactly that line reverts to
     the robotic voice. Recording both numbers is what lets a later stage notice. */
  const body = `/* voice-clips: ${note} */\n`
    ...
    + `window.AIB_VOICE_CLIPS = ${safeJson({
      ...
      scope: SCOPE, n, corpus: items.length, mp3Bytes, ms, clips,
    })};\n`;
