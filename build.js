...
/* ============================================================================
   ...
   Three targets, one composer:
       ...
       node build.js --public-config
                                  → inline config.public.js instead: keyless,
                                    same-origin /api/ endpoints (the public URL)
   ...
   The config seam has three outcomes, not two, and they are three different
   DEPLOYMENTS rather than three degrees of the same one:

       (default)         config.js        a key in the page. A laptop you own,
                                          on a stand. Never handed out.
       --no-config       nothing          no credential, no network. The
                                          emailed file, the booth stick, the
                                          air-gapped venue, and the container.
       --public-config   config.public.js keyless same-origin endpoints, served
                                          by a proxy that holds the keys. The
                                          public, unauthenticated URL.

   --public-config WINS over the default, so it never merely adds to a key that
   was already there — it replaces the source outright. That precedence runs in
   the safe direction on purpose: the mistake it forgives (asking for a public
   build on a machine that happens to have a config.js) produces a keyless
   artifact, and the alternative precedence would produce a public URL with a
   credential in view-source. --no-config and --public-config are NOT in
   conflict and compose fine; both say "the key does not go in", and
   --public-config additionally says what goes in instead.
...
/* ── --public-config ──────────────────────────────────────────────────────
   Refuse to build if config.public.js has stopped being secret-free.

   WHY THIS NEEDS A GUARD OF ITS OWN, when config.js has three already.

   Every existing guard against a leaked key is a fence around the BUILD
   CONTEXT: config.js is gitignored, it is named in .dockerignore, the
   Dockerfile's builder stage copies an explicit allowlist that excludes it,
   and a stage-0 audit fails if it reaches the context at all. Each of those
   works by keeping the file AWAY from the build.

   Not one of them can protect config.public.js, and not by oversight — by
   construction. This file is TRACKED (so gitignore does not apply), it must
   ENTER the Docker context and be named in the builder's COPY allowlist (so
   .dockerignore and the allowlist cannot exclude it), and it is deliberately
   inlined into an artifact served at a public, unauthenticated URL. The whole
   defence for config.js is unavailable here. So the guard has to be about
   CONTENT instead of location, and it has to live at the one point every
   --public-config build passes through, which is this file.

   The failure it catches is an ordinary, well-meaning edit. Someone debugging
   a proxy that is not answering tries the key directly in config.public.js to
   isolate the problem, it works, and the line survives into a commit. There is
   now a live credential in git history AND in view-source of a page anyone on
   the internet can open. Nothing else in this repo would say a word: the build
   is green, the page is correct, the container audit passes (the artifact's
   grep in the Dockerfile catches the key PREFIX, which is the backstop — but
   it only runs on a docker build, and the public deck is also built and
   deployed by hand).

   Two checks, because each covers exactly what the other cannot:

     1. THE RAW TEXT, for key-shaped strings. It has to be the text and not the
        parsed object, because this file is inlined VERBATIM — comments and
        all — into the artifact. A key pasted into a comment is not in the
        config object, ships to every visitor anyway, and an object walk would
        never see it. Same pattern as the Dockerfile's guard 3, and the same
        length bound for the same reason: a bare `sk_` must not fail the build
        on prose.

     2. THE PARSED OBJECT, for populated credential FIELDS. A regex over text
        cannot tell a real setting from the word `apiKey` inside a paragraph
        explaining why there is no apiKey — and this file is mostly such
        paragraphs, so a text-only check would either be noisy or be written so
        loosely it stops catching anything. Evaluating the assignment answers
        the precise question instead: after this file runs, does AIB_CONFIG
        contain a populated credential anywhere in it?

   Evaluating it also settles, for free, the one thing nothing else checks:
   that the file PARSES. It is spliced in without ever being parsed by this
   build (exactly as config.js is), so a stray comma here is not a build error
   — it is a SyntaxError in the inlined <script>, which aborts that script
   before window.AIB_CONFIG is ever assigned. The deck then loads, looks
   perfect, and silently has no endpoints at all. */
const PUBLIC_CONFIG = 'config.public.js';

/* Matches the ElevenLabs prefix (sk_ + >=16) and the Anthropic one (sk-ant-). */
const KEY_SHAPED = /sk_[A-Za-z0-9]{16,}|sk-ant-/;

/* Field names that may not carry a value in a file that ships to the public.
   Compared lowercased, so voiceId/VOICE_ID/voiceid are all one rule.

   voiceId is on this list and it is the entry people argue with, so: it is not
   a secret, but it is not a PUBLIC INPUT either. src/voice.js sends text to
   the proxy; which voice renders it is decided server-side next to the key. In
   the page it becomes a parameter a visitor can edit in dev tools to bill any
   voice on the account. Not a credential leak — a spend leak. */
const FORBIDDEN_FIELDS = new Set([
  'apikey', 'voiceid', 'token', 'secret', 'password',
  'authorization', 'credential', 'bearer',
]);

function assertPublicConfigClean() {
  const die = lines => {
    console.error('');
    console.error(`REFUSING TO BUILD: ${PUBLIC_CONFIG} is not safe to publish.`);
    console.error('');
    lines.forEach(l => console.error(l));
    console.error('');
    console.error(`${PUBLIC_CONFIG} is TRACKED IN GIT and is inlined into an artifact served`);
    console.error('at a public, unauthenticated URL. It may contain endpoints and nothing else.');
    console.error('');
    console.error('If you need a credential to reproduce something locally, put it in config.js');
    console.error('(gitignored) and build with no flags — never in this file.');
    console.error('');
    process.exit(1);
  };

  if (!exists(PUBLIC_CONFIG)) {
    die([
      `--public-config was asked for and there is no ${PUBLIC_CONFIG}.`,
      '',
      'Unlike config.js this file is TRACKED, so a clone has one. Its absence',
      'means either it was deleted, or this is a Docker build whose builder stage',
      'did not COPY it — the builder names an explicit allowlist, so a new file',
      'has to be added to that line or it simply is not there.',
      '',
      'Fix: restore it from git, or add it to the COPY in the Dockerfile.',
    ]);
  }

  const src = read(PUBLIC_CONFIG);

  // 1. the raw text — catches a key in a COMMENT, which ships verbatim.
  const hit = src.match(KEY_SHAPED);
  if (hit) {
    die([
      `It contains something shaped like an API key (matched ${JSON.stringify(hit[0].slice(0, 12) + '…')}).`,
      '',
      'This check reads the RAW TEXT, not the config object, because this file is',
      'inlined verbatim — comments included. A key commented out is still a key in',
      'the page every visitor loads, and commenting it out is what people do',
      'instead of deleting it.',
    ]);
  }

  // 2. the parsed object — catches a populated credential FIELD.
  let cfg;
  try {
    const vm = require('vm');
    /* A bare context: no require, no process, no fs. This file is ours and is
       about to be inlined into a browser, where it will have far less than
       this — but the sandbox is what makes "evaluate it" a check rather than
       an execution path worth worrying about. */
    const sandbox = { window: {} };
    vm.createContext(sandbox);
    vm.runInContext(src, sandbox, { filename: PUBLIC_CONFIG, timeout: 1000 });
    cfg = sandbox.window.AIB_CONFIG;
  } catch (e) {
    die([
      `It does not parse or does not run: ${e.message}`,
      '',
      'This matters more than it looks. The file is spliced in WITHOUT being',
      'parsed by this build, so a syntax error here is not a build failure — it is',
      'a SyntaxError in the inlined <script> at page load, which aborts before',
      'window.AIB_CONFIG is assigned. The deck then loads, looks entirely correct,',
      'and has no endpoints at all: every question falls back to the local',
      'knowledge base and every line to the pre-rendered voice, silently.',
    ]);
  }

  if (!cfg || typeof cfg !== 'object') {
    die([
      'Running it did not assign window.AIB_CONFIG to an object.',
      '',
      'The file must be a single `window.AIB_CONFIG = { ... };` assignment, the',
      'same shape as config.js and config.example.js.',
    ]);
  }

  /* Walk it whole rather than checking two known paths. The point is to catch
     a field somebody ADDED, and a check that only looks where fields already
     are cannot see a new one. */
  const problems = [];
  const walk = (node, trail) => {
    for (const [k, v] of Object.entries(node)) {
      const path = trail ? `${trail}.${k}` : k;
      if (v && typeof v === 'object' && !Array.isArray(v)) {
        /* `headers` is called out by name because its documented use in
           config.example.js is literally an Authorization bearer, and the
           field name carrying it is the caller's choice — so the denylist
           below cannot be relied on to catch it. Any entry at all is wrong
           here. */
        if (k.toLowerCase() === 'headers' && Object.keys(v).length) {
          problems.push(`${path} — has ${Object.keys(v).length} header(s); its documented use is an Authorization bearer, which is a credential`);
        }
        walk(v, path);
        continue;
      }
      if (FORBIDDEN_FIELDS.has(k.toLowerCase()) && v !== '' && v != null && v !== false) {
        problems.push(`${path} — is populated, and must not be in a public build`);
      }
      if (typeof v === 'string' && KEY_SHAPED.test(v)) {
        problems.push(`${path} — its value is shaped like an API key`);
      }
    }
  };
  walk(cfg, '');
...
function build({ withConfig = true, publicConfig = false, want3d = false,
  wantArtifact = false, withVoice = true, requireVoice = false } = {}) {
  ...
  /* ── which config, if any, gets inlined ──────────────────────────────────
     One seam, three outcomes. Resolved to a NAMED SOURCE rather than straight
     to a string, because everything downstream — the build log, and above all
     the "a key is baked into this file" warning — needs to know WHICH of the
     two files it got, not merely that it got one.

     That warning is the reason this is not a one-liner. It used to fire on
     `configJs` being non-empty, which was exactly right while config.js was
     the only file that could fill it. Leave it that way and every public build
     prints "a key is baked into this file — do not hand it out" about an
     artifact whose entire purpose is to be handed out, with no key in it. A
     warning that cries wolf on the one build type it does not apply to is
     worse than no warning: it is the first thing an operator learns to scroll
     past, and it is the last thing they should.

     --public-config takes precedence over a present config.js. See the header:
     the precedence runs toward the keyless outcome deliberately. */
  const configSource = publicConfig ? PUBLIC_CONFIG
    : (withConfig && exists('config.js') ? 'config.js' : null);
  /* Before a byte is written, and before the voice check below for the same
     reason: a build that is going to be refused must not leave a half-correct
     dist/ behind. */
  if (publicConfig) assertPublicConfigClean();
  const configJs = configSource ? read(configSource) : '';
  const configHasKey = configSource === 'config.js';
  ...
  const configNote = configHasKey ? 'INLINED (config.js — WITH KEY)'
    : configSource ? `PUBLIC (${PUBLIC_CONFIG} — keyless, same-origin /api/)`
      : 'omitted';
  console.log(`  ${scenes} scenes · ${kb} knowledge entries · config ${configNote}`);
  ...
  /* Only for config.js. A --public-config build is keyless BY DESIGN and is
     meant to be handed out; warning about it would teach the operator to
     ignore this line on the builds where it is true. */
  if (configHasKey) console.log('  ⚠  a key is baked into this file — do not hand it out');
  if (publicConfig) {
    console.log('  ·  keyless: the endpoints are relative, so they resolve against whatever');
    console.log('     origin serves the page. Needs the aib-proxy behind /api/ — see');
    console.log('     docs/DEPLOY-PROXY.md. Without it the deck degrades, it does not break.');
  }
...
build({
  ...
  publicConfig: process.argv.includes('--public-config'),
